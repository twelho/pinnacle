...
All prerequisites, including a present TPM with an active SHA-256 bank, a clean (all-zero) PIN target PCR, and a reachable next loader that passes Secure Boot verification, is checked before the prompt, so a misconfiguration never wastes a PIN entry. A non-zero PCR (e.g. a boot loop that re-entered pinnacle) aborts rather than extending twice. Any fatal condition prints why and reboots, rather than falling through to another boot entry. At the prompt, `Ctrl+U` clears the entry and `ESC` aborts, also by rebooting.
